...
public:
    ...
    bool WriteZNIOSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZNIOSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZNIOSeed_deprecated(uint256& seed);
    bool EraseZNIOSeed();
    bool EraseZNIOSeed_deprecated();

    bool WriteZNIOCount(const uint32_t& nCount);
    bool ReadZNIOCount(uint32_t& nCount);
