...
// The zNIO block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zNIO is a hash of the serial
...
{
    //Create an output returning the zNIO that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZNIOOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zNIO output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zNIO", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZNIOOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zNIO output", __func__);
...
{
    CzNIOTracker* znioTracker = pwallet->znioTracker.get();
    CMintMeta meta;
    if (!znioTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    znioTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a NIO stake is the outpoint
