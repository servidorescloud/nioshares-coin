...
/**
 * Nioshares RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZnioSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setznioseed(const UniValue& params, bool fHelp);
extern UniValue getznioseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdznio(const UniValue& params, bool fHelp);
extern UniValue dzniostate(const UniValue& params, bool fHelp);
