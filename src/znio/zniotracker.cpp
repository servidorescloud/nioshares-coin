...
CzNIOTracker::CzNIOTracker(std::string strWalletFile)
{
...
CzNIOTracker::~CzNIOTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzNIOTracker::Init()
{
...
bool CzNIOTracker::Archive(CMintMeta& meta)
{
...
bool CzNIOTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzNIOTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzNIOTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzNIOTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzNIOTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzNIOTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzNIOTracker::GetSerialHashes()
{
...
CAmount CzNIOTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzNIOTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzNIOTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzNIOTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzNIOTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzNIOTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzNIOTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzNIOTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzNIOTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzNIOTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzNIOTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzNIOWallet* zNIOWallet)
{
    bool iszNIOWalletInitialized = (NULL != zNIOWallet);
    CMintMeta meta;
    ...
    if (! iszNIOWalletInitialized)
        zNIOWallet = new CzNIOWallet(strWalletFile);
    meta.isSeedCorrect = zNIOWallet->CheckSeed(dMint);
    if (! iszNIOWalletInitialized)
        delete zNIOWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzNIOTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzNIOTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzNIOTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzNIOTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzNIOTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzNIOTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzNIOWallet* zNIOWallet = new CzNIOWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zNIOWallet);
        }
        delete zNIOWallet;
        LogPrint("zero", "%s: added %d dznio from DB\n", __func__, listDeterministicDB.size());
    }
