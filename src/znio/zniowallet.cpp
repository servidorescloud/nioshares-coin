...
CzNIOWallet::CzNIOWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing znio seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZNIOSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZNIOSeed_deprecated()) {
                    LogPrintf("%s: Updated zNIO seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old znio seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of znio wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzNIOWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZNIOCount(nCountLastUsed);
    else if (!walletdb.ReadZNIOCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzNIOWallet::Lock()
{
    seedMaster = 0;
}

void CzNIOWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzNIOWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZNIO(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzNIOWallet::LoadMintPoolFromDB()
{
...
void CzNIOWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzNIOWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzNIOWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->znioTracker->HasPubcoinHash(pMint.first)) {
...
bool CzNIOWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZNIO(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to znioTracker which also adds to database
    pwalletMain->znioTracker->Add(dMint, true);
...
void CzNIOWallet::SeedToZNIO(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzNIOWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzNIOWallet::GenerateDeterministicZNIO(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzNIOWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzNIOWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzNIOWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
