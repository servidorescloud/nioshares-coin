...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZNIOModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->znioTracker->HasMintTx(hash))) {
            //zNIO stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZNIO;
...
bool IsZNIOType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZNIO:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZNIO || type == TransactionRecord::MNReward) {
