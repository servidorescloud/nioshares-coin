...
#include "ui_zniocontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->znioTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->znioTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zNIO.";
            else if (!mint.isSeedCorrect)
                strReason = "The zNIO seed used to mint this zNIO is not the same as currently hold in the wallet";
            else
